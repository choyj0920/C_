...
void DrawImage::draw_divied_strech_puzzle(int x, int y, int sx, int sy, int wx, int wy, const TCHAR* imagefilename, const POINT set[][8])
{
	HBITMAP hbit = (HBITMAP)LoadImage(NULL, imagefilename, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
	//HBITMAP hbit = LoadBitmap(_hInst, lpmbpName);
	HBITMAP oldbit = (HBITMAP)SelectObject(_memDC, hbit);
	...
	for (int i = 0; i < wx; i++) {
		for (int j = 0; j < wy; j++) {
			StretchBlt(_onewrDC, x + perw_s * set[j][i].x, y + perh_s * set[j][i].y, perw_s - 3, perh_s - 3, _memDC, perw * i, perh * j, perw, perh, SRCCOPY);
		}
	}
